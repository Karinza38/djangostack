...
class DjangoStack(TaskSet):
    ...
    def __init__(self, projectName, apacheConfigName='apache_site',
                 scmType='mercurial', databaseName=None, databaseUser=None,
                 databasePassword=None, djangoProjectPath=None,
                 djangoAdminUser=DEFAULT_DJANGO_ADMIN_USER,
                 djangoAdminEmail=DEFAULT_DJANGO_ADMIN_EMAIL,
                 djangoAdminPassword=DEFAULT_DJANGO_ADMIN_PASSWORD,
                 databaseDumpType='SQL',
                 djangoVersionNumber=""):
    ...
    def createDatabaseUser(self):
        postgresql_role_ensure(self.DATABASE_USER,
                               self.DATABASE_PASSWORD,
                               createdb=True)

    @task_method
    def createDatabase(self):
        postgresql_database_ensure(self.DATABASE_NAME,
    ...
    def createApacheSite(self):
        put("%s" % self.APACHE_CONFIG_NAME,
            "/etc/apache2/sites-enabled/%s" % self.PROJECT_NAME,
            use_sudo=True)
    ...
    def createDjangoAdminUser(self):
        ...
        sudo("cd %s;echo \"from django.contrib.auth.models import User;"
             "User.objects.create_superuser('%s', '%s', '%s')\""
             "| ./manage.py shell" %
             (self.DJANGO_PROJECT_PATH,
              self.DJANGO_ADMIN_USER,
              self.DJANGO_ADMIN_EMAIL,
